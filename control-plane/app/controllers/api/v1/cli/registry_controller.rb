...
module Api
  module V1
    module Cli
      class RegistryController < Api::V1::Cli::BaseController
        before_action :authenticate_cli_access!
        rate_limit to: 20, within: 1.minute, name: "cli_registry_push_auth", by: :cli_rate_limit_key, with: :render_rate_limited, only: :push_auth
        ...
        def push_auth
          ...
          if organization.active_runtime_project.gcp? && organization.organization_bundle.nil? && organization.gar_repository_name.blank?
          ...
          auth = ::Cli::RegistryPushAuthIssuer.new(organization: organization).call

          render json: {
            registry_host: auth.fetch(:registry_host),
            repository_path: auth.fetch(:repository_path),
        ...
        rescue Cli::RegistryPushAuthIssuer::Error => error
          render_error("registry_auth_failed", error.message, status: :unprocessable_entity)
        end
