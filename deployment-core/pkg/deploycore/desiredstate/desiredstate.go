...
// Exported aliases expose the JSON document model to adapters while preserving
// the existing internal type names used by the implementation and tests.
type DesiredStateJSON = desiredStateJSON
type EnvironmentJSON = environmentJSON
type ServiceJSON = serviceJSON
type ServicePortJSON = servicePortJSON
type HealthcheckJSON = healthcheckJSON
type VolumeMountJSON = volumeMountJSON
type TaskJSON = taskJSON
type IngressJSON = ingressJSON
type IngressTLSJSON = ingressTLSJSON
type IngressRouteJSON = ingressRouteJSON
type IngressMatchJSON = ingressMatchJSON
type IngressTargetJSON = ingressTargetJSON
type NodePeerJSON = nodePeerJSON
