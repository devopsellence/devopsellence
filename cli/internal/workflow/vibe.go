...
//go:embed all:vibe_templates/rails-app/root
...
func (a *App) generateVibeRailsApp(ctx context.Context, target, templateURL string, force bool) error {
	if _, err := a.LookPath("rails"); err != nil {
		if !errors.Is(err, exec.ErrNotFound) {
			return fmt.Errorf("check rails on PATH: %w", err)
		}
		return generateBundledVibeRailsApp(target)
	}
...
func generateBundledVibeRailsApp(target string) error {
	appName := vibeSlug(filepath.Base(target))
	replacements := map[string]string{
		"{{APP_NAME}}":   appName,
		"{{APP_MODULE}}": vibeRailsModuleName(appName),
	}
	return copyEmbeddedVibeTemplate("vibe_templates/rails-app/root", target, replacements)
}
...
func vibeRailsModuleName(appName string) string {
	parts := strings.Split(vibeSlug(appName), "-")
...
func vibeStackPromptLines(stack string) []string {
	...
	case vibeIndexPHPStack:
		return []string{
			"Stay inside the index.php baseline: PHP 8.4, nginx latest with PHP-FPM, one public/index.php entrypoint, PDO SQLite, no build step, Docker, and a .mise.toml for local tool versions.",
	...
	default:
		return []string{
			"Stay inside the blessed Rails MVP baseline: Rails 8.1, SQLite by default, Hotwire, Tailwind, Solid Queue/Cache/Cable, Active Storage only when needed, Minitest, Docker, and a .mise.toml for local tool versions.",
