...
func TestRootSkillListOnlyIncludesDevopsellenceSkill(t *testing.T) {
	var stdout bytes.Buffer
	cmd := NewRootCommand(bytes.NewBuffer(nil), &stdout, &stdout, t.TempDir())
	cmd.SetOut(&stdout)
	cmd.SetErr(&stdout)
	...
	for _, want := range []string{"devopsellence"} {
		if !stringSliceContains(ids, want) {
			t.Fatalf("skill ids = %v, missing %q", ids, want)
		}
	}
	if stringSliceContains(ids, "app") {
		t.Fatalf("skill ids = %v, app skill should not be bundled by devopsellence", ids)
	}
}
